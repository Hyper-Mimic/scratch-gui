// Adapted from ScratchAddons' recolor-custom-blocks addon for HyperMimic's addon system.
// Changes:
//  - updateAllBlocks now comes from ../custom-block-shape/update-all-blocks.js (HyperMimic signature: (vm, workspace, blockly))
//  - editor-theme3 interop (getAddonValue / callSharedMethod) replaced with direct reads of
//    the shared SettingsStore singleton (same values editor-theme3 reads via addon.settings)
//  - addon.self.dir asset loading replaced with addon.self.getResource()
//  - updateBlockColors(this) calls fixed to pass the actual declaration block
//  - removed debug console.warn calls
...
import SettingsStore from "../../settings-store-singleton.js";

// --- editor-theme3 interop -----------------------------------------------
// HyperMimic doesn't have ScratchAddons' shared-method API. editor-theme3's
// "text" setting and enabled state are read from the shared settings store.
const theme3Enabled = () => SettingsStore.getAddonEnabled("editor-theme3");
const theme3TextMode = () => SettingsStore.getAddonSetting("editor-theme3", "text");
const theme3IsColoredTextMode = () => {
  const mode = theme3TextMode();
  return mode === "colorOnWhite" || mode === "colorOnBlack";
};
...
export default async function ({ addon, console }) {
  ...
  // HyperMimic's updateAllBlocks expects (vm, workspace, blockly)
  const refreshAllBlocks = () => {
    updateAllBlocks(addon.tab.traps.vm, addon.tab.traps.getWorkspace(), Blockly);
  };
  ...
  const setProcedureButtonColor = (iconElement, colors) => {
    ...
    if (theme3Enabled() && theme3IsColoredTextMode()) {
  ...
  const setDeclarationColor = (block, colors, prefix) => {
    ...
    if (Blockly.registry) {
      ...
      // As theme3 manually edits this color, we use our own fallback coloring here.
  ...
  // Method attached to createAllInputs which optionally hides color prefixes.
  // The prefix lives in procCode_ (e.g. "motion:my block"). We detect it directly
  // from procCode_ instead of relying on recolorCustomBlock, so the option works
  // for ANY block that carries a color prefix (including plain Scratch-generated
  // procedure blocks), not only ones recolored through this addon's menu.
  const getProcCodePrefix = (procCode) => {
    const idx = procCode.indexOf(":");
    if (idx <= 0) return null;
    const candidate = procCode.substring(0, idx);
    // Only treat known category names or #rrggbb hex values as color prefixes.
    if (categoryColors.includes(candidate) || isHexColor(candidate)) return candidate;
    return null;
  };
  const customCreateAllInputs = (oldCreateAllInputs) => {
    return function (...args) {
      ...
      updateBlockColors(this);
      if (addon.settings.get("hide-color-prefix")) {
        const prefix = getProcCodePrefix(this.procCode_ || "");
        if (prefix) {
  ...
  // Patch the shared ProcedureUtils.createAllInputs_ so procedures_call,
  // procedures_prototype AND procedures_declaration (the definition block shown
  // in the code area) all honor the hide-color-prefix option. Patch it directly
  // when available; otherwise fall back to per-block injection.
  if (Blockly.ScratchBlocks && Blockly.ScratchBlocks.ProcedureUtils && Blockly.ScratchBlocks.ProcedureUtils.createAllInputs_) {
    const oldShared = Blockly.ScratchBlocks.ProcedureUtils.createAllInputs_;
    Blockly.ScratchBlocks.ProcedureUtils.createAllInputs_ = customCreateAllInputs(oldShared);
  }
  // Per-block fallback for procedures_call / procedures_prototype (covers forks
  // that don't expose the shared ProcedureUtils method).
  for (const fn of ["procedures_call", "procedures_prototype"]) {
    if (!Blockly.Blocks[fn] || !Blockly.Blocks[fn].createAllInputs_) continue;
    if (Blockly.registry) {
  ...
  addon.settings.addEventListener("change", () => refreshAllBlocks());
