...
function recursiveFillBlock(block, fill = null) {
  ...
  // 官方 Blockly 新版用 block.pathObject.svgPath，TurboWarp scratch-blocks 用 block.svgPath_（兼容两者）
  const pathEl = block.pathObject && block.pathObject.svgPath ? block.pathObject.svgPath : block.svgPath_;
  if (pathEl) setFill(pathEl.style);
...
class HeatmapManager {
  ...
  hideHeatmapFn() {
    ...
    // Collect every block that needs clearing, from three sources:
    //   1. blocks we previously tracked
    //   2. current timer top-level blocks (all profiling modes)
    //   3. any block still carrying the heatmap class, scanned directly
    const ids = new Set();
    this.coloredBlocks.forEach((id) => ids.add(id));
    this.updateBlocks((block) => ids.add(block.id));
    if (typeof workspace.getAllBlocks === "function") {
      for (const block of workspace.getAllBlocks()) {
        const root = block.getSvgRoot && block.getSvgRoot();
        if (root && root.classList.contains("sa-debugger-heatmap")) {
          ids.add(block.id);
        }
      }
    }

    // Reset each block INDEPENDENTLY. recursiveFillBlock() ends with block.applyColour(),
    // which can throw on an edge-case block; a throw must not abort the rest, otherwise
    // only the first block gets cleared (one block per click).
    for (const id of ids) {
      ...
      try {
        ...
        // Ignore per-block failures so the rest still get cleared.
      }
    }
