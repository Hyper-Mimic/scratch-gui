...
export default async function createTimingTab({ debug, addon, console, msg }) {
  ...
  function createExportButton() {
    ...
    exportButton.element.addEventListener("click", () => {
      ...
      // Create CSV headers (localized)
      const headers = [
        msg("timing-label"),
        `${msg("timing-total-time")} (ms)`,
        `${msg("timing-average-time")} (ms)`,
        config.showRatioTime ? msg("timing-ratio-time") : msg("timing-percent-time"),
        msg("timing-call-count"),
      ];
  ...
  const tab = debug.createHeaderTab({
    text: msg("tab-timing"),
    icon: addon.self.getResource("/icons/timing.svg"),
  });
  ...
  return {
    ...
    footerButtons: [lineByLineButton, heatmapButton],
    // 语言切换时重设所有常驻文本（由 userscript.js 的 reenabled 钩子调用）
    updateLocalizedText: () => {
      ...
      const headerKeyByClass = {
        ...
        "timing-total-time": "timing-total-time",
        "timing-average-time": "timing-average-time",
        "timing-percent-time": "timing-percent-time",
        "timing-call-count": "timing-call-count",
      };
      tableHeader.querySelectorAll("span").forEach((span) => {
        const cls = span.classList[0];
        if (headerKeyByClass[cls]) {
          span.textContent = msg(headerKeyByClass[cls]);
        }
      });
      // 占比/比值表头随设置更新
      percentHeader.textContent = config.showRatioTime
        ? msg("timing-ratio-time")
        : msg("timing-percent-time");
      // 占位符
      tableRows.placeholderElement.textContent = msg("timing-no-timers");
    },
    show: () => {
      tableRows.show();
