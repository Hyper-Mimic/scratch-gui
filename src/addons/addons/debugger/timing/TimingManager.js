class TimingManager {
  ...
  startTimer(label, targetId = null, blockId = null) {
    // Use a default key for empty/undefined/null labels
    const isEmptyLabel = label === "" || label === undefined || label === null;
    if (isEmptyLabel) {
  ...
  stopTimer(label) {
    // Use the same default key for empty/undefined/null labels
