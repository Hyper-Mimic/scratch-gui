...
export default async function ({ addon, console }) {
  const recolorSvg = (svg) => {
    return svg
      ...
      .replace(/%primary1%/g, addon.settings.get("accentColor"))
      .replace(
        /%primary2%/g,
        alphaBlend(addon.settings.get("input"), multiply(addon.settings.get("accentColor"), { a: 0.7 }))
      )
      .replace(
        /%primary3%/g,
        alphaBlend(addon.settings.get("input"), multiply(addon.settings.get("accentColor"), { a: 0.375 }))
      )
      .replace(/%white%/g, textColor(addon.settings.get("accentColor")))
